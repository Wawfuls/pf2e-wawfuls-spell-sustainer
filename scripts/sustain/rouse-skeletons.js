...
export async function handleTemplatedSustain(caster, sustainingEffect, spellConfig) {
  ...
  if (existingTemplateId) {
    ...
    if (existingTemplate) {
      console.log(`[PF2e Spell Sustainer] Removing existing template for ${spellName}`);
  ...
  try {
    ...
    if (initialTemplate) {
      console.log(`[PF2e Spell Sustainer] Started interactive template placement for ${spellName}`);
    } else {
